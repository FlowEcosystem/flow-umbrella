...
export const useGroupsStore = defineStore('groups', () => {
  ...
  async function fetch({ limit: l, offset: o } = {}) {
    ...
    error.value = null
    try {
      const data = await groupsApi.list({ limit: lim, offset: off })
  ...
  async function update(id, patch) {
    const data = await groupsApi.update(id, patch)
  ...
  function replaceOne(group) {
    const idx = items.value.findIndex((item) => item.id === group.id)
